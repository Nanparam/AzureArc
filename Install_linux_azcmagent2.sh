...
function log_failure() {
  ...
  if [ "${cloud}" = "AzureUSGovernment" ]; then
    ...
    if [ ${use_wget} -eq 1 ]; then
        if [ -n "${proxy}" ]; then
            wget -qO- -e use_proxy=yes -e https_proxy=${proxy} --method=PUT --body-data="$logBody" ${his_endpoint}/log &> /dev/null || true
        else
...
# $2 is where to output downloaded file
    ...
    if [ ${use_curl} -eq 1 ]; then  
        if [ -n "${proxy}" ]; then
            curl --proxy ${proxy} "$1" -o "$2"
        else
            curl "$1" -o "$2"
        fi
    elif [ ${use_wget} -eq 1 ]; then  
        if [ -n "${proxy}" ]; then
            wget -e use_proxy=yes -e http_proxy=${proxy} "$1" -O "$2"
        else
            wget "$1" -O "$2"
        fi
    fi
...
if [ ${use_curl} -eq 1 ]; then
    imds_response=$(curl "http://169.254.169.254/metadata/instance/compute?api-version=2019-06-01" -f -s -H "Metadata: true" --connect-timeout 1 --noproxy "*")
else
    imds_response=$(wget "http://169.254.169.254/metadata/instance/compute?api-version=2019-06-01" --header="Metadata: true" --connect-timeout=1 --tries=1 --quiet --no-proxy)
fi
if [ $? -eq 0 ]; then
...
if [ $apt -eq 1 ]; then
    ...
    install_cmd="apt"
    if [ -n "${altdownloadfile}" ]; then
	apt install "${tempdir}/azcmagent.deb"
    else
        # clean up previous microsoft configuration
        apt -y remove packages-microsoft-prod || true
        ...
        dpkg -i "${tempdir}/packages-microsoft-prod.deb"
        if [ -n "${proxy}" ]; then
            apt-get update
        else
            apt-get update
        ...
        if [ -n "${desired_version}" ]; then
            ...
            if [ -n "${proxy}" ]; then
                apt install -y azcmagent=${desired_version}*
            else
                apt install -y azcmagent=${desired_version}*
            fi
        else
    ...
    install_cmd="zypper"
    if [ -n "${altdownloadfile}" ]; then
	zypper install -y "${tempdir}/azcmagent.rpm"
    else
        # clean up previous microsoft configuration
        zypper remove -y packages-microsoft-prod || true
        ...
        rpm -i "${tempdir}/packages-microsoft-prod.rpm"
        ...
        if [ -n "${desired_version}" ]; then
            ...
            if [ -n "${proxy}" ]; then
                zypper --gpg-auto-import-keys --non-interactive install -y -l azcmagent=${desired_version}
            else
                zypper --gpg-auto-import-keys --non-interactive install -y -l azcmagent=${desired_version}
            fi
        else
            if [ -n "${proxy}" ]; then
                zypper --gpg-auto-import-keys --non-interactive install -y -l azcmagent
            else
                zypper --gpg-auto-import-keys --non-interactive install -y -l azcmagent
            fi
        fi
    ...
    if [ -n "${altdownloadfile}" ]; then
        if [ $localinstall -eq 0 ]; then
            ${yum} -y install "${tempdir}/azcmagent.rpm"
        else
            ${yum} -y localinstall "${tempdir}/azcmagent.rpm"
        fi
    else
        if [ -n "${rpm_distro}" ]; then
            # clean up previous microsoft configuration
            yum -y remove packages-microsoft-prod || true
        ...
        if [ -n "${desired_version}" ]; then
            ...
            if [ -n "${proxy}" ]; then
                ${yum} -y install azcmagent-${desired_version}
            else
                ${yum} -y install azcmagent-${desired_version}
            fi
        else
            if [ -n "${proxy}" ]; then
                ${yum} -y install azcmagent
            else
                ${yum} -y install azcmagent
            fi
        fi
...
if [ -n "${proxy}" ]; then
    ...
    azcmagent config set proxy.url ${proxy}
